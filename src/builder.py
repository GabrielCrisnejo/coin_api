...
from src.settings import DB_URL, PLOTS_STORE
...
def compute_features(group):
    ...
    try:
        ...
        logger.error(f"Error computing features for group {group['coin_id'].iloc[0]}: {e}")
        return group
...
def train_and_evaluate_model(df, model_type="linear"):
    ...
    try:
        ...
        logger.error(f"Error training and evaluating the {model_type} model: {e}")
...
def plot_price_trends(df):
    ...
    try:
        for coin_id, group in df.groupby("coin_id"):
            ...
            plt.legend()
            plt.grid()
            plt.savefig(f"{PLOTS_STORE}/{coin_id}_price_trend.png")
            plt.close()
    except Exception as e:
        logger.error(f"Error plotting price trends: {e}")
